...
class MemoryManager:
    """
    Memory management class for handling vector storage and retrieval
    ...
    def store_memory(self, text: str, metadata: Dict[str, Any], importance: float = 1.0) -> str:
        """
        Store a memory in the vector database with importance scoring
        ...
        """
        try:
            # Add timestamp and importance score to metadata
            metadata.update({
            ...
            # Always include the memory text in metadata
            metadata["memory"] = text
