...
/**
 * Full‑screen intro animation for Kuro AI (Next.js compatible)
 * - Animated gradient / radial glow background
 * - Floating particle / bokeh field (CSS keyframes – very light)
 * - Cycling headline phrases with smooth fade / scale (Framer Motion)
 */
export default function KuroIntro({ phrases = ["Let's Imagine", "Let's Build", 'Kuro AI'], cycleMs = 1800 }) {
  const [index, setIndex] = useState(0);
  const mounted = useRef(false);

  // Pre-generate particles once (stable across re-renders)
  const particles = useMemo(() => {
    return Array.from({ length: 28 }).map((_, i) => ({
      id: i,
      left: Math.random() * 100, // vw percent
      top: Math.random() * 100,  // vh percent
  ...
  return (
    <div className="fixed inset-0 z-[9999] overflow-hidden select-none">
      ...
      {/* Local component styles (keyframes) */}
  <style>{`
        ...
        @keyframes kuroFloat {
          ...
          25% { transform: translateY(-12vh) translateX(4vw) scale(1.15); }
          50% { transform: translateY(-24vh) translateX(-2vw) scale(0.9); }
          75% { transform: translateY(-36vh) translateX(3vw) scale(1.1); }
          100% { transform: translateY(-48vh) translateX(0) scale(1); }
        }
      `}</style>
    </div>
  );
...
// Helper hook (optional export) to gate intro display externally
